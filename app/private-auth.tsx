...
export default function PrivateAuthScreen() {
  ...
  const handleAuthentication = useCallback(() => {
    ...
    if (!password.trim()) {
      Alert.alert(
        'Champ requis',
        'Veuillez saisir votre mot de passe d\'accès.',
        [{ text: 'OK', style: 'default' }]
      );
      return;
    }

    if (password === PRIVATE_LINK_PASSWORD) {
      setIsLoading(true);
      
      // Simulation d'authentification avec gestion d'erreur
      setTimeout(() => {
        try {
          router.push('/private-links');
        } catch (error) {
          ...
          Alert.alert(
            'Erreur',
            'Une erreur est survenue lors de l\'accès aux liens privés.',
            [{ text: 'OK', style: 'default' }]
          );
        }
      }, 500);
      ...
      >
        ...
        >
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>Accès Privé</Text>
          ...
          <View style={[styles.content, keyboardVisible && styles.contentWithKeyboard]}>
            ...
            <Text style={styles.title}>Liens Privés</Text>
            <Text style={styles.subtitle}>
              Accédez à vos liens de réunion privés et sécurisés
            </Text>

            {!keyboardVisible && (
              <View style={styles.benefitsContainer}>
                <Text style={styles.benefitsTitle}>Avantages des Liens Privés :</Text>
                <Text style={styles.benefitItem}>• Liens de réunion sécurisés</Text>
                <Text style={styles.benefitItem}>• Mots de passe de réunion gérés</Text>
                <Text style={styles.benefitItem}>• Organisation par catégories</Text>
                <Text style={styles.benefitItem}>• Support prioritaire</Text>
              </View>
            )}

            <View style={styles.passwordContainer}>
              <Text style={styles.passwordLabel}>Mot de passe d'accès</Text>
            ...
            >
              ...
              >
                <Text style={styles.authButtonText}>
                  {isLoading ? 'Authentification...' : 'Accéder aux Liens Privés'}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
            ...
            {!keyboardVisible && (
              <TouchableOpacity style={styles.helpButton}>
                <Text style={styles.helpText}>
                  Besoin d'aide pour gérer vos liens privés ?
                </Text>
              </TouchableOpacity>
            )}
          </View>
